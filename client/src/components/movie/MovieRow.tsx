...
export default function MovieRow({ title, movies, genreSlug, isLoading = false, showProgress = false }: MovieRowProps) {
    ...
    >
      ...
      <div className="px-4 md:px-8 mb-5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg md:text-xl lg:text-2xl font-bold text-text-primary tracking-tight">{title}</h2>
        </div>
        {genreSlug && (
          <Link
            href={`/genre/${genreSlug}`}
            className="flex-shrink-0 flex items-center gap-1.5 px-4 py-1.5 text-xs font-semibold text-brand border border-brand/30 rounded-full hover:bg-brand hover:text-bg-primary transition-all duration-200 cursor-pointer"
          >
            See All
